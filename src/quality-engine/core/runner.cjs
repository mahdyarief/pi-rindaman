...
class Runner {
  ...
  finish() {
    ...
    const reportPath = configuredReportPath
      ...
      : path.join(process.cwd(), ".pi-rindaman", "report.md");
    ...
    console.log(
      `${yellow > 0 ? YELLOW : GREEN}  🟡 Yellow Issues (Recommend to handle with human observation): ${yellow}${RESET}`,
    );
    console.log(
      `${totalArch > 0 ? DIM : GREEN}  🏛️ Architectural Debt (Informational): ${totalArch}${RESET}`,
    );
    if (totalArch > 0) {
      console.log(
        `${DIM}     - God Functions: ${archIssues.god}\n     - Bloated Methods: ${archIssues.bloated}\n     - Tolerable Debt: ${archIssues.tolerable}${RESET}`,
      );
    }
