...
const FORBIDDEN_VAR_NAMES =
  /\b(?:const|let|var)\s+(data|result|temp|info|stuff|items?|obj|arr|list)\s*=/;
...
module.exports = {
  ...
  run(context, reporter) {
    ...
    // TS Regex
    const FORBIDDEN_VAR_NAMES_TS =
      /\b(?:const|let|var)\s+(data|result|temp|info|stuff|items?|obj|arr|list)\s*=/;
    const FORBIDDEN_DESTRUCTURE_TS = /\bconst\s+\{\s*data\s*[,}](?!\s*:\s*\w)/;

    // Go Regex
    // Matches e.g. "func Name(data any)" or "var res result" or "item := ..."
    const FORBIDDEN_VAR_NAMES_GO =
      /:=|var\s+|func\s+.*\(\s*(data|result|res|temp|info|stuff|items?|obj|arr|list)\b/;
