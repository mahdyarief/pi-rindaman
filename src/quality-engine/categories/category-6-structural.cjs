    ...
    for (const file of context.tsFiles) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        const classMatch = line.match(
          /^(?:export\s+)?(?:abstract\s+)?class\s+(\w+(?:Factory|Strategy|Adapter|Provider|Processor|Handler|Manager|Helper))\b/,
        );
        if (classMatch) {
          ...
          for (let j = i + 1; j < Math.min(i + 60, lines.length); j++) {
            ...
            if (
              /^(?:public\s+|async\s+|static\s+)*\w+\s*\(/.test(l) &&
              !l.startsWith("//") &&
              !l.startsWith("constructor")
            ) {
              publicMethodCount++;
            }
          ...
          if (publicMethodCount === 1) {
            reporter.warn(
              `Single-method class "${className}" at ${file}:${i + 1} — consider a plain function`,
            );
            clean = false;
          }
        }
        ...
        if (wrapperMatch) {
          ...
          if (
            ...
            reporter.warn(
              `Wrapper function "${fnName}" at ${file}:${i + 1} only delegates — consider inlining`,
            );
            clean = false;
          }
        }
