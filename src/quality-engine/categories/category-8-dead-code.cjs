  ...
  run(context, reporter) {
    ...
    for (const file of context.tsFiles) {
      ...
      lines.forEach((line, i) => {
        if (
          /^\s*\/\/\s*(const|let|var|function|return|if|for|import|export|type|interface)\b/.test(
            line,
          )
        ) {
        ...
        if (/\b(isDebug|debugMode|showBoundingBox|showHitArea|devMode)\b/.test(line)) {
    ...
    reporter.note(
      "Tip: run 'npx ts-prune' for unused exports and 'npx biome check .' for unused imports.",
    );
  },
};
