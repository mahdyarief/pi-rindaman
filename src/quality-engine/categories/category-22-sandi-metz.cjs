  ...
  run(context, reporter) {
    ...
    function isNonLogicLine(l, isGo) {
      ...
      return (
        ...
        /^(?:export\s+)?(?:interface|type)\s/.test(l) ||
        /^\s*[a-zA-Z?]+\s*[?:].*;\s*$/.test(l)
      );
    }
    ...
    for (const file of context.sourceFiles) {
      ...
      for (const line of lines) {
        ...
        if (!isGo) {
          if (
            /^(?:export\s+)?(?:interface|type\s+\w+[\s<][^=]*=\s*\{)/.test(l) &&
            l.includes("{")
          ) {
            inTypeBlock = true;
            typeDepth = 0;
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (parenIdx !== -1) {
          ...
          if (paramCount > TOLERABLE.params) {
            reporter.arch(
              "BLOATED",
              `Too many parameters: ${paramCount} in "${fnName}" | ${file}:${i + 1}`,
            );
            overallStatus = "BLOATED";
          } else if (paramCount > EXCELLENT.params) {
            reporter.arch(
              "TOLERABLE",
              `Many parameters: ${paramCount} in "${fnName}" | ${file}:${i + 1}`,
            );
            if (overallStatus !== "BLOATED") overallStatus = "TOLERABLE";
          }
        }
        ...
        if (funcLogicLines > funcLimitTol) {
          reporter.arch(
            "BLOATED",
            `Function too long: ${funcLogicLines} logic lines "${fnName}" | ${file}:${i + 1}`,
          );
          overallStatus = "BLOATED";
        } else if (funcLogicLines > funcLimitExc) {
          reporter.arch(
            "TOLERABLE",
            `Function growing: ${funcLogicLines} logic lines "${fnName}" | ${file}:${i + 1}`,
          );
          if (overallStatus !== "BLOATED") overallStatus = "TOLERABLE";
        }
      }
